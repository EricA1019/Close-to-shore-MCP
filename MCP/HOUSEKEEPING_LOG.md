...
- MCP/TOOLS/mcp_server.py (still referenced by task: Start MCP Python stdlib HTTP server Server)
...
- Consider deprecating `Start MCP Python stdlib HTTP server Server` if not relied on.
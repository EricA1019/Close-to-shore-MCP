...
static func get_level_state(level_state_key : String):
	if not has_game_state(): 
		return null
	var game_state := get_or_create_state()
	if level_state_key.is_empty(): return null
	if level_state_key in game_state.level_states:
		return game_state.level_states[level_state_key]
	else:
		var new_level_state = load("res://godot_project/scripts/level_state.gd").new()

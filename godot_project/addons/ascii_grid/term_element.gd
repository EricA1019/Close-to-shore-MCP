...
## Blits the node and it's children to the provided buffer.
...
## Supports both TermBuffer (legacy shader) and AsciiCanvasBuffer (new canvas) rendering.
func blit_to_buffer(buffer) -> void:
	if buffer is TermBuffer:
		# Legacy shader-based rendering
...
## Canvas version of _blit_self_under for new rendering system.
func _blit_self_under_canvas(_buffer) -> void:
	# Default: delegate to legacy method for compatibility
	if _buffer is TermBuffer:
		_blit_self_under(_buffer)
...
## Canvas version of _blit_self_over for new rendering system.
func _blit_self_over_canvas(_buffer) -> void:
	# Default: delegate to legacy method for compatibility
	if _buffer is TermBuffer:
		_blit_self_over(_buffer)
